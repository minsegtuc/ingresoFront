            ...
            <div className='flex flex-col lg:mt-8 mt-2 w-full'>
                {
                    seccionesPermitidas.map((seccion, index) => {
                        if (seccion.nombre !== "MODULOS") {
                            ...
                            return <a href={seccion.enlace} className={`w-full bg-[#f0f0f0] text-black border-t-2 border-b-2 py-2 flex justify-center hover:bg-black hover:text-white`}>{seccion.nombre}</a>
                        }
                    })
                }
            </div>
